...
def load_sprite(file_path: Path) -> dict[str, Any]:
    ...
    sprite: dict[str, Any] = {
        ...
        "pivotX": data.get("pivotX"),
        "pivotY": data.get("pivotY"),
        "primitives": [],
    }
    ...
    if sprite["pivotX"] is None:
        sprite["pivotX"] = int(sprite["width"]) // 2
    if sprite["pivotY"] is None:
        sprite["pivotY"] = int(sprite["height"]) // 2
    require_int(f"{file_path}: pivotX", sprite["pivotX"], 0, SPRITE_MAX_VALUE)
    require_int(f"{file_path}: pivotY", sprite["pivotY"], 0, SPRITE_MAX_VALUE)
...
def validate_sprite_packable(sprite: dict[str, Any]) -> None:
    ...
    require_int(f"{sprite_id}.pivotX", sprite["pivotX"], 0, SPRITE_MAX_VALUE)
    require_int(f"{sprite_id}.pivotY", sprite["pivotY"], 0, SPRITE_MAX_VALUE)
...
""")
    ...
    for sprite in sprites:
        ...
        pivot_x = int(sprite["pivotX"])
        pivot_y = int(sprite["pivotY"])
