...
void screen_join_game_render(Framebuffer* framebuffer, const AppState* app) {
    ...
    int title_width = font_measure_text(font, 3, "Join Game");
    int body_width = font_measure_text(font, 2, "Coming soon");

    ui_draw_label(framebuffer, font, (app->screenWidth - title_width) / 2, app->screenHeight / 2 - 80, 3, 0x27312bffu, "Join Game");
    ui_draw_label(framebuffer, font, (app->screenWidth - body_width) / 2, app->screenHeight / 2 + 8, 2, 0x3d405bffu, "Coming soon");
    ui_draw_button(framebuffer, font, screen_join_game_back_rect(app), "Back");
}
...
void screen_join_game_handle_tap(AppState* app, int x, int y) {
    ...
    rect = screen_join_game_back_rect(app);
