...
DEFAULT_INPUT_DIR = PROJECT_ROOT / "src" / "assets" / "music"
DEFAULT_OUTPUT_FILE = PROJECT_ROOT / "src" / "audio" / "generated_music.c"
DEFAULT_HEADER_FILE = PROJECT_ROOT / "src" / "audio" / "generated_music.h"
...
def emit_generated_c_header() -> str:
    return """// Generated by apps/android/tools/pack_music.py. Do not edit manually.
...
def emit_generated_h(patterns: list[PackedMusic]) -> str:
    lines = [
    ...
    for index, pattern in enumerate(patterns):
        suffix = "," if index < len(patterns) - 1 else ","
        lines.append(f"    {music_id_symbol(pattern.music_id)} = {index}{suffix}")

    lines.append("    MUSIC_ID_COUNT")
    lines.extend(
        [
            "} MusicId;",
            "",
    ...
    for pattern in patterns:
        lines.append(f"#define MUSIC_{make_symbol(pattern.music_id)} {music_id_symbol(pattern.music_id)}")
...
def emit_registry(patterns: list[PackedMusic]) -> str:
    ...
    lines.append("")
    lines.append("const char* generated_music_id_name(MusicId music_id)")
    lines.append("{")
    lines.append("    switch (music_id)")
    lines.append("    {")

    for pattern in patterns:
        lines.append(f"        case {music_id_symbol(pattern.music_id)}: return \"{pattern.music_id}\";")
...
def emit_music_c(patterns: list[PackedMusic]) -> str:
    chunks = [emit_generated_c_header()]
...
def convert_all(input_dir: Path, output_file: Path, header_file: Path) -> None:
    ...
    header_file.write_text(emit_generated_h(patterns), encoding="utf-8")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pack Cat Chess music JSON files into compressed C source/header.")
    ...
    parser.add_argument("--header", type=Path, default=DEFAULT_HEADER_FILE, help="Generated C header file.")
    args = parser.parse_args()

    try:
        convert_all(args.input_dir, args.output, args.header)
